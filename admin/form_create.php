  ...
  <div class="panel">
    ...
    <form method="post" action="">
      ...
      <div class="form-group">
        ...
        <input class="form-control" type="text" name="form_name" value="<?= htmlspecialchars($formNameInput, ENT_QUOTES, 'UTF-8') ?>" required>
      </div>

      <div class="form-group">
        <label class="form-label">所属站点（每站点仅一个主表单）</label>
      ...
      <table class="table table-fields" id="fields-table">
        ...
        <?php foreach ($fields as $i => $f): ?>
          ...
          <tr class="field-row" data-builtin="<?= $isBuiltin ? '1' : '0' ?>">
            <td>
              <input class="form-control" type="text" name="fields[<?= $rowId ?>][key]" value="<?= htmlspecialchars($key, ENT_QUOTES, 'UTF-8') ?>" <?= $isBuiltin ? 'readonly' : '' ?> required>
      ...
      <div class="form-group">
        ...
        <div class="checkbox-group">
          <label class="checkbox-item"><input type="checkbox" name="enable_ga4" value="1" <?= $flagsInput['enable_ga4'] ? 'checked' : '' ?>> enable_ga4</label>
          <label class="checkbox-item"><input type="checkbox" name="enable_ads" value="1" <?= $flagsInput['enable_ads'] ? 'checked' : '' ?>> enable_ads</label>
          <label class="checkbox-item"><input type="checkbox" name="enable_enhanced_conversion" value="1" <?= $flagsInput['enable_enhanced_conversion'] ? 'checked' : '' ?>> enable_enhanced_conversion</label>
          <label class="checkbox-item"><input type="checkbox" name="require_gclid" value="1" <?= $flagsInput['require_gclid'] ? 'checked' : '' ?>> require_gclid</label>
        </div>
      </div>

      <h3>站点跟踪配置（GA4 / Ads）</h3>
      <div class="form-grid">
        <div><label class="form-label">GA4 Measurement ID</label><input class="form-control" type="text" name="ga4_measurement_id" value="<?= htmlspecialchars((string) $trackingInput['ga4_measurement_id'], ENT_QUOTES, 'UTF-8') ?>"></div>
        <div><label class="form-label">GA4 API Secret</label><input class="form-control" type="text" name="ga4_api_secret" value="<?= htmlspecialchars((string) $trackingInput['ga4_api_secret'], ENT_QUOTES, 'UTF-8') ?>"></div>
        <div><label class="form-label">Ads Conversion ID</label><input class="form-control" type="text" name="ads_conversion_id" value="<?= htmlspecialchars((string) $trackingInput['ads_conversion_id'], ENT_QUOTES, 'UTF-8') ?>"></div>
        <div><label class="form-label">Ads Conversion Label</label><input class="form-control" type="text" name="ads_conversion_label" value="<?= htmlspecialchars((string) $trackingInput['ads_conversion_label'], ENT_QUOTES, 'UTF-8') ?>"></div>
        <div class="full"><label class="form-label">该站点收件邮箱（用于邮件通知）</label><input class="form-control" type="text" name="smtp_to_email" value="<?= htmlspecialchars((string) $trackingInput['smtp_to_email'], ENT_QUOTES, 'UTF-8') ?>"></div>
      </div>

      <div class="actions">
        <button class="btn btn-primary" type="submit">创建并进入编辑</button>
