...
?>
<style>
.container{max-width:960px;margin:0 auto}.table td{border-bottom:1px solid #e5e7eb;padding:10px;vertical-align:top}.table td:first-child{width:220px;background:#f9fafb;font-weight:bold}
.status{padding:2px 8px;border-radius:999px;font-size:12px}.ok{background:#dcfce7;color:#166534}.fail{background:#fee2e2;color:#991b1b}.pending{background:#fef3c7;color:#92400e}
...
<div class="container panel">
    ...
    <table class="table">
        ...
        <tr><td>自定义字段</td><td>
            <?php if (!$payload): ?>
                <span style="color:#6b7280">无</span>
            <?php else: ?>
                <?php foreach ($payload as $key => $val): ?>
                    <div><strong><?= htmlspecialchars((string) ($labelMap[(string) $key] ?? $key), ENT_QUOTES, 'UTF-8') ?></strong>：<?= htmlspecialchars(is_scalar($val) ? (string) $val : json_encode($val, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES), ENT_QUOTES, 'UTF-8') ?></div>
                <?php endforeach; ?>
            <?php endif; ?>
        </td></tr>
        ...
        $cls = static fn(string $s): string => $s === 'success' ? 'ok' : ($s === 'failed' ? 'fail' : 'pending');
        ?>
